...
When a run time error is encountered, the board will flash its LEDs in a distinctive pattern to let you know that an error has occurred and that the program has stopped running.
<span>
...
<<code title="Example of a run time error">>
#include "mbed.h"
...
<<info title="Debug to a serial console">>
...
For more information on using the USB Serial port, see the [[SerialPC]] page.
<</info>>

<<code title="Example showing serial terminal debug messages>>
#include "mbed.h"
...
<<code Example showing variable value debugging>>
...
All the different techniques mentionned previously are sometimes inefficient and have strong disadvantages:
...
Try out the [[http://mbed.org/handbook/CMSIS-DAP-MDK|mbed CMSIS-DAP interface with Keil MDK]], if you need full debug capabilities:
